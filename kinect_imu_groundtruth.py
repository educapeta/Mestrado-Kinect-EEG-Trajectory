"""Ground truth 3D da mao: Kinect + MediaPipe Hands + MPU6050.

O Kinect e a referencia metrica principal. O IMU fornece orientacao e
predicao de movimento entre frames validos do Kinect; ele nao e integrado
cegamente quando existe uma medida de profundidade confiavel.

Instalacao:
    python -m pip install opencv-python mediapipe numpy pykinect2

Requisitos de hardware:
- Kinect v2 com Kinect for Windows SDK 2.0 instalado
- ESP32 enviando:
  timestamp_us,roll,pitch,yaw,ax_g,ay_g,az_g,gx_dps,gy_dps,gz_dps

Controles:
- C: captura origem e inicia calibracao do referencial
- B: coleta 2 s parado para estimar bias do acelerometro
- ESPACO: captura o tabuleiro para alinhar as cameras
- R: reseta origem e estado
- ESC: encerra

A webcam auxiliar e opcional: ela e usada para indicar deteccao/oclusao da
mao em uma segunda vista, mas nao entra na escala metrica do ground truth.
"""
...
# medida da camera. Com a mao parada, o residuo do acelerometro e composto
# so de bias+ruido, entao a janela permite estimar o bias com precisao.
IMU_ZERO_WINDOW = 15
            ...
            try:
                ...
                with self.lock:
                    self.latest = ImuSample(
                        int(values[0]), received_at, float(values[1]), float(values[2]),
                        float(values[3]), accel.astype(float), gyro.astype(float)
                    )
...
class PositionFusion:
    ...
    Quando a camera some, a integracao continua com o bias JA CORRIGIDO, o
    que segura o drift por segundos. `absolute_position` e a posicao no
    camera space do Kinect (ancora = primeira medida valida).
    """
    ...
    def calibrate_palm_direction(self, sample, palm_direction):
        ...
        self.palm_reference_direction = palm_direction.copy()
        return True
